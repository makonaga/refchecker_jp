...
LABELS = ["Contradiction", "Entailment", "Neutral"]  ## for cyberagent/xlm-roberta-large-jnli-jsick
...
class RepCChecker(CheckerBase):
    def __init__(
        ...
        prompt_style='chatml',  # 互換性のため保持するが実際は使用しない
        selected_token=-1,
        device=0,
        batch_size=16
    ):
        ...
        ----------
        model : str, optional
            The name or identifier of the RepC backbone to use, defaults to 'Rakuten/RakutenAI-7B-instruct.
        ...
        prompt_style : str, optional
            The style of the prompt to use, defaults to 'chatml'. (注意: RakutenAI-7B-instructでは実際には使用されない)
        ...
        self.prompt_style = prompt_style  # 互換性のため保持
    ...
    def get_prompt(self, prompt_style, question, premise, hypothesis):
        """
        RakutenAI-7B-instruct用のプロンプト生成
        prompt_styleパラメータは互換性のため受け取るが使用しない
        """
        prompt = f"""質問: {question}
前提: {premise}
仮説: {hypothesis}

提供された前提と仮説の論理関係を判定してください。選択肢はEntailment（含意）、Neutral（中立）、Contradiction（矛盾）です。
...
if __name__ == "__main__":
    claims = ["富士山の標高は3776メートルです", "東京都の人口は約1400万人です"]
    references = ["富士山は日本最高峰の山で、標高は3,776メートルです。静岡県と山梨県にまたがって位置しています。",
                  "東京都の人口は2023年時点で約1,400万人となっており、日本で最も人口の多い都道府県です。"]
    questions = ["富士山の高さはどのくらいですか？", "東京都の人口はどのくらいですか？"]
    checker = RepCChecker()
    ret = checker._check(claims=claims, references=references, responses=["", ""], questions=questions)
    print(ret)